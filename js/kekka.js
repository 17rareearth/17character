...
window.onload = function() {
  ...
  // 3. スコア計算とキャラクター選出（重み付きランダム方式）
  const sigma = 1.3; // 分布の広がり調整用（数値が大きいほどバラけます）
  ...
  const weights = elements.map(el => {
    ...
    // スコア表示用に暫定スコア（%）を保存
    el.score = Math.min(98, Math.round(similarity * 100));
    return similarity;
  });
  ...
  // 当選したキャラを配列の先頭[0]に持ってくる（表示のため）
  const winner = elements.splice(winnerIndex, 1)[0];
  elements.unshift(winner);

  // 残りのキャラをスコア順に並び替えて、2位と3位を決める
  const secondThird = elements.slice(1).sort((a, b) => b.score - a.score);
  elements.splice(1, elements.length - 1, ...secondThird);

  // 適合度の見栄え調整（1位を高く、2・3位を少し下げる）
  elements[0].score = Math.max(85, Math.min(98, elements[0].score + 15));
  elements[1].score = Math.min(elements[0].score - 5, elements[1].score);
  elements[2].score = Math.min(elements[1].score - 5, elements[2].score);

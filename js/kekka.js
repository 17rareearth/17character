...
window.onload = function() {
  ...
  // 3. 【新ロジック】ベクトルの距離（最小二乗法）による適合度判定
  elements.forEach(el => {
    // ユーザーの各軸の平均値を算出（12問で各軸3問ずつと想定し、0〜3のスケールに補正）
    // ※2, 1, -1, -2 のスコアをキャラの 0〜3 スケールに合わせるための計算です
    const normalize = (val) => (val + 6) / 4; // -6〜+6 の範囲を 0〜3 に変換
    ...
    // 各軸の「ズレ」を計算し、二乗して足し合わせる（距離が近いほど似ている）
    // 重み付け：S(こだわり)とC(信念)の不一致はより「似ていない」と判断する
    let distance =

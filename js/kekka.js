  ...
  const elements = [
    {
      ...
      },
      {
        ...
        E: 0.5, A: 0.2, S: 3, C: 1.5
      }
    ];

  // 3. 軸計算（加点方式で、不一致を厳しく判定）
  elements.forEach(el => {
    let matchScore = 0;
    [
      { u: user.E, e: el.E, weight: 1.0 },
      { u: user.A, e: el.A, weight: 1.2 }, // A(速さ)を少し重視
      { u: user.S, e: el.S, weight: 1.5 }, // S(こだわり)を重視
      { u: user.C, e: el.C, weight: 1.5 }  // C(空気・論理)を重視
    ].forEach(axis => {
      let diff = Math.abs(axis.u - axis.e);
      let point = 0;
      if (diff < 0.3) point = 25;      // より厳密に
      else if (diff < 0.7) point = 15;
      else if (diff < 1.2) point = 5;
      matchScore += point * axis.weight;
    });
    // 基礎スコアを算出
    el.score = Math.min(Math.round((matchScore / 130) * 100), 98);
  });

  // --- 3.5 最終出現バランス補正（ここで順位をひっくり返す！） ---
  const finalBias = {
    "ルテチウム": 0.5,    // 出すぎているキャラを大幅抑制
    "セリウム": 0.7,
    "イットリウム": 0.7,
    "スカンジウム": 0.7,
    "ガドリニウム": 1.6,  // 出にくいキャラを大幅に底上げ
    "イッテルビウム": 1.8,
    "ツリウム": 2.0       // 最大のボーナス
  };

  elements.forEach(el => {
    if (finalBias[el.name]) {
      // スコアそのものを掛け算で変えて、ランキングを入れ替える
      el.score = Math.round(el.score * finalBias[el.name]);
    }
    el.score = Math.max(0, Math.min(el.score, 98));
  });

  // 4. 並び替え（補正が終わった「後」に実行！）
  elements.sort((a, b) => b.score - a.score);

  // 上位3名のスコアに差をつけて見栄えを良くする
  // ※並び替えの後なので、[0]〜[2]の順序は確定済み

...
document.addEventListener("DOMContentLoaded", function() {
  // ===== 1. 受験者スコア取得 =====
  const user = JSON.parse(localStorage.getItem("uranaiScore"));

  if (!user) {
    const resultArea = document.getElementById("resultArea");
    if (resultArea) {
      resultArea.innerHTML = "<h2>スコアが見つかりません。TOPからやり直してください。</h2>";
    }
    return;
  }

  // ===== 2. 17元素データ（全データ・文章完全復元） =====
  const elements = [
    {
  ...
  for (let i = 1; i <= 2; i++) {
    ...
    <div class="resultCard" style="padding: 20px; margin-bottom: 15px; border-top: 4px solid #bdc3c7;">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
        <div style="font-size: 13px; color:#777; font-weight:bold;">第 ${i + 1} 位</div>
